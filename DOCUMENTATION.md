...
- Fecha: 2026-05-16
  - Cambio: Se agrego reemplazo real para CV y foto de perfil. Los bloques CMS documentales ahora devuelven `documentUrl` versionado con `v=<documentId>` y la descarga publica de bloques usa `Cache-Control: no-store`, evitando que el navegador abra un CV viejo por cache. Al reemplazar `contact.cv` o `site.profile-photo`, el backend elimina el documento anterior si existe. Tambien se agrego `V22__profile_photo_content_block.sql` con bloques `site.profile-photo` ES/EN, Home permite subir la foto desde `EditMode` y Header/Home comparten la URL nueva con fallback al asset estatico.
  - Archivos: `backend/src/main/resources/db/migration/V22__profile_photo_content_block.sql`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/mapper/publiccontent/PublicContentBlockMapper.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/service/impl/PublicContentBlockServiceImpl.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/controller/PublicContentBlockController.java`, `backend/src/test/java/com/fernandogferreyra/portfolio/backend/ApiIntegrationTest.java`, `frontend/src/app/services/profile-photo.service.ts`, `frontend/src/app/components/home/*`, `frontend/src/app/components/header/*`, `frontend/src/app/components/contact/contact.component.html`, `frontend/src/app/components/home/home.component.spec.ts`, `docs/features/profile-photo-and-cv-replacement*.md`, `DOCUMENTATION.md`, `docs/handoff-control-center.md`.
  - Decision: Modelar la foto como bloque CMS global `site.profile-photo` con documento asociado, reutilizando la base documental existente en vez de crear una tabla o endpoint nuevo. Para el CV, borrar el documento previo y versionar la URL resuelve tanto metadata vieja como cache del navegador.
  - Validacion: `npx tsc -p tsconfig.app.json --noEmit` OK; `npx tsc -p tsconfig.spec.json --noEmit` OK; tests focalizados Home/Header/Contact OK (`8 SUCCESS`); `npm run build:ci` OK con warnings conocidos; backend `mvnw.cmd -DskipTests clean test-compile` OK; `DocumentFileServiceImplTest` OK (`3` tests); `git diff --check` OK. `ApiIntegrationTest#replacingContactCvDeletesPreviousDocumentAndVersionsPublicUrl` quedo bloqueado localmente por Docker/Testcontainers no disponible.
  - Proximos pasos: Correr CI, desplegar y validar visualmente con admin: subir CV nuevo, abrirlo, subir otro y confirmar que no abre el anterior; subir foto de perfil y confirmar Inicio/Header.

- Fecha: 2026-05-16
  - Cambio: Se preparo `fix/durable-cv-documents` para resolver de forma permanente el CV roto en deploy. El backend agrega `V21__durable_document_contents.sql` y una tabla `document_contents` con binario `BYTEA` por documento; cada subida admin guarda ahora el archivo en filesystem como cache y tambien en PostgreSQL como copia durable. Las descargas de CMS/CV, credenciales, proyectos y skills usan filesystem primero y caen a la copia DB si el archivo fisico falta. La baja admin tambien elimina la copia durable y ya no queda bloqueada si el archivo fisico ya desaparecio.
  - Archivos: `backend/src/main/resources/db/migration/V21__durable_document_contents.sql`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/domain/documents/entity/DocumentContentEntity.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/repository/documents/DocumentContentRepository.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/service/DocumentFileService.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/service/impl/DocumentFileServiceImpl.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/service/impl/DocumentServiceImpl.java`, servicios de descarga publica/admin, `backend/src/test/java/com/fernandogferreyra/portfolio/backend/service/impl/DocumentFileServiceImplTest.java`, `backend/src/test/java/com/fernandogferreyra/portfolio/backend/ApiIntegrationTest.java`, `docs/features/durable-document-storage*.md`, `DOCUMENTATION.md`, `docs/handoff-control-center.md`.
  - Decision: Usar PostgreSQL como storage durable pragmatico para documentos chicos del portfolio, evitando depender de un disco persistente externo para el CV. No se abre endpoint publico generico ni se cambian permisos; los documentos siguen exponiendose solo por asociaciones publicadas existentes.
  - Validacion: `mvnw.cmd -DskipTests clean test-compile` OK; `DocumentFileServiceImplTest` OK (`3` tests); `git diff --check` OK. `ApiIntegrationTest` focalizado queda bloqueado localmente por Docker/Testcontainers no disponible.
  - Proximos pasos: Correr CI, mergear/promover/deployar backend con `V21`, re-subir y asociar el CV en produccion una vez para crear la copia durable; luego validar `/api/content-blocks/contact.cv/es/document`.

- Fecha: 2026-05-15
  - Cambio: Se cerro una pasada visual pre-deploy sobre navbar, themes y Contacto. El header queda mas compacto y alineado en desktop, sin accesos GitHub/LinkedIn duplicados porque esos canales viven en Contacto. El selector de themes ahora muestra opciones compactas por nombre/swatch, sin descripciones largas. `EX` pasa a fondo negro con rojo puro, `CMD` queda forzado a blanco/negro, `WB` se renombra visualmente como `GREY` con escala de grises comoda, y se suma el theme `GRID` azul nocturno con rejilla. La rejilla de fondo global queda animada por variables de theme. Contacto cambia los canales a tiles grandes tipo glass sobre grilla, priorizando visualmente GitHub, LinkedIn y Gmail.
  - Archivos: `frontend/src/app/components/header/header.component.html`, `frontend/src/app/components/header/header.component.scss`, `frontend/src/app/components/contact/contact.component.scss`, `frontend/src/app/app.component.scss`, `frontend/src/styles.scss`, `frontend/src/app/data/portfolio.data.ts`, `frontend/src/app/data/portfolio.models.ts`, `frontend/src/app/services/theme.service.ts`, `docs/features/pre-deploy-navbar-theme-polish.md`, `DOCUMENTATION.md`, `docs/handoff-control-center.md`
  - Decision: Conservar el id interno `themeLightWorkbench` para no romper preferencias ya guardadas en `localStorage`; el cambio a `GREY` es visible. El video local adjunto no es accesible desde el runner, por lo que la direccion visual se tomo de la captura y la referencia web: grilla azul animada y tiles de contacto.
  - Validacion: `npx tsc -p tsconfig.app.json --noEmit` OK; `npx tsc -p tsconfig.spec.json --noEmit` OK; `cmd.exe /c "npm run build:ci"` OK con warnings de budget conocidos; `cmd.exe /c "npm test -- --watch=false --browsers=ChromeHeadless"` OK (`47 SUCCESS`); `cmd.exe /c "set JAVA_HOME=C:\Program Files\Java\jdk-17&& mvnw.cmd -DskipTests package"` OK.
  - Proximos pasos: Revalidar visualmente desktop/mobile el header autenticado, selector de themes y Contacto antes de mergear; luego abrir PR hacia `develop`, esperar CI verde, promover `develop` a `main`, correr release y deploy.

- Fecha: 2026-05-15
  - Cambio: Se cerro la ultima pasada de `EditMode` para `Skills`: cada skill ahora puede eliminarse desde la accion visible junto a `Editar` con confirmacion visual tematizada, subir un icono propio usando el storage documental existente (`purpose=skill-icon`) y configurar su color principal. El backend agrega `V20__skills_icon_color_and_delete.sql`, `icon_document_id`, `accent_color`, baja admin `DELETE /api/admin/skills/{id}` y descarga de icono por `/api/skills/{id}/icon`. Tambien se evita crear multiples categorias default sin editar: si ya existe una `Nueva categoria` vacia, el alta vuelve a seleccionarla. Al borrar documentos usados como icono, el backend desvincula las skills afectadas.
  - Archivos: `backend/src/main/resources/db/migration/V20__skills_icon_color_and_delete.sql`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/domain/skills/entity/SkillEntity.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/dto/skills/*`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/mapper/skills/SkillCatalogMapper.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/service/SkillCatalogService.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/service/impl/SkillCatalogServiceImpl.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/controller/SkillController.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/controller/admin/SkillAdminController.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/repository/skills/SkillRepository.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/service/impl/DocumentServiceImpl.java`, `frontend/src/app/models/skills.models.ts`, `frontend/src/app/services/skill-admin.service.ts`, `frontend/src/app/components/skills/skills.component.ts`, `frontend/src/app/components/skills/skills.component.html`, `frontend/src/app/components/skills/skills.component.scss`, `frontend/src/app/components/skills/skills.component.spec.ts`, `frontend/src/app/components/home/home.component.spec.ts`, `docs/features/edit-mode-skills-catalog.md`, `docs/features/edit-mode-skills-catalog-verification.md`, `docs/features/edit-mode-skills-catalog-adversarial-review.md`, `DOCUMENTATION.md`, `docs/handoff-control-center.md`
  - Decision: Reutilizar `documents` para iconos en vez de crear storage paralelo. El icono subido se trata como asset visual no sensible; si en el futuro se quiere ocultar borradores, se puede separar preview admin por Blob autenticado.
  - Validacion: `cmd.exe /c "set JAVA_HOME=C:\Program Files\Java\jdk-17&& mvnw.cmd -DskipTests clean test-compile"` OK; `npx tsc -p tsconfig.app.json --noEmit` OK; `npx tsc -p tsconfig.spec.json --noEmit` OK; `cmd.exe /c "npm test -- --watch=false --browsers=ChromeHeadless --include src/app/components/skills/skills.component.spec.ts --include src/app/components/home/home.component.spec.ts"` OK (`5 SUCCESS`); `cmd.exe /c "npm run build:ci"` OK con warnings de budget conocidos.
  - Proximos pasos: Revalidar visualmente en navegador el flujo completo: subir icono, cambiar color, borrar skill, confirmar modal y limpiar categorias duplicadas creadas durante pruebas.

- Fecha: 2026-05-15
  - Cambio: Se corrigio la UX de `Skills` en `EditMode` segun revision visual. El bloque superior `skills.hero` deja de mostrarse como editable en esta pantalla y `Enfoque tecnico` queda solo como resumen automatico derivado de categorias publicadas. Al entrar en `EditMode` o crear una skill/categoria, la pagina fuerza la vista expandida para que los editores sean visibles; el carrusel animado no muestra acciones de edicion. `Nueva categoria` ahora selecciona una categoria pendiente existente si ya fue creada, evitando duplicados por clicks repetidos, y la edicion/eliminacion de categorias queda visible en la vista expandida.
  - Archivos: `frontend/src/app/components/skills/skills.component.ts`, `frontend/src/app/components/skills/skills.component.html`, `DOCUMENTATION.md`, `docs/handoff-control-center.md`, `docs/features/edit-mode-skills-catalog-verification.md`
  - Decision: Mantener la edicion de Skills centrada solo en el catalogo real. El hero y el recuadro de enfoque no son superficies de edicion contextual en esta etapa.
  - Validacion: `npx tsc -p tsconfig.app.json --noEmit` OK; `npx tsc -p tsconfig.spec.json --noEmit` OK; `cmd.exe /c "npm test -- --watch=false --browsers=ChromeHeadless --include src/app/components/skills/skills.component.spec.ts --include src/app/components/home/home.component.spec.ts"` OK (`5 SUCCESS`); `cmd.exe /c "npm run build:ci"` OK con warnings de budget conocidos.
  - Proximos pasos: Limpiar desde la UI las categorias duplicadas creadas durante la prueba usando `Eliminar categoria`, y revalidar visualmente que `Nueva categoria` abre el formulario visible en la vista expandida.

- Fecha: 2026-05-15
  - Cambio: Se implemento el catalogo backend-first de `Skills`. El backend agrega la migracion `V19__skills_catalog.sql`, tablas `skill_categories` y `skills`, seed ES/EN desde el catalogo vigente, endpoint publico `GET /api/skills` y endpoints admin `GET/POST/PATCH /api/admin/skills` + `POST/PATCH/DELETE /api/admin/skill-categories`. En frontend, `Skills` consume el catalogo real, mantiene el CMS solo para hero, y en `EditMode` permite crear/editar skills con nivel, icono, nombre, descripcion, categoria, tags, orden y publicacion. Tambien permite crear/editar/eliminar categorias; al eliminar una categoria, el backend reasigna sus skills a `Otras`. `Enfoque tecnico` y `Home > Stack y enfoque` ahora derivan de categorias/skills publicadas reales, incluyendo `Soft skills`.
  - Archivos: `backend/src/main/resources/db/migration/V19__skills_catalog.sql`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/domain/skills/entity/SkillEntity.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/domain/skills/entity/SkillCategoryEntity.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/repository/skills/SkillRepository.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/repository/skills/SkillCategoryRepository.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/dto/skills/*`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/mapper/skills/SkillCatalogMapper.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/service/SkillCatalogService.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/service/impl/SkillCatalogServiceImpl.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/controller/SkillController.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/controller/admin/SkillAdminController.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/controller/admin/SkillCategoryAdminController.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/security/SecurityConfiguration.java`, `frontend/src/app/models/skills.models.ts`, `frontend/src/app/services/skill.service.ts`, `frontend/src/app/services/skill-admin.service.ts`, `frontend/src/app/components/skills/skills.component.ts`, `frontend/src/app/components/skills/skills.component.html`, `frontend/src/app/components/skills/skills.component.scss`, `frontend/src/app/components/skills/skills.component.spec.ts`, `frontend/src/app/components/home/home.component.ts`, `frontend/src/app/components/home/home.component.spec.ts`, `docs/features/edit-mode-skills-catalog.md`, `docs/features/edit-mode-skills-catalog-verification.md`, `docs/features/edit-mode-skills-catalog-adversarial-review.md`, `DOCUMENTATION.md`, `docs/handoff-control-center.md`
  - Decision: Usar tablas dedicadas porque el modelo de skill ya excede `public_content_blocks`. No se borran los bloques CMS `skill.*` en esta etapa; quedan como legado/no fuente principal. Las skills nuevas se crean por idioma activo y no se traducen automaticamente todavia.
  - Validacion: `cmd.exe /c "set JAVA_HOME=C:\Program Files\Java\jdk-17&& mvnw.cmd -DskipTests clean test-compile"` OK; `npx tsc -p tsconfig.app.json --noEmit` OK; `npx tsc -p tsconfig.spec.json --noEmit` OK; `cmd.exe /c "npm test -- --watch=false --browsers=ChromeHeadless --include src/app/components/skills/skills.component.spec.ts --include src/app/components/home/home.component.spec.ts"` OK (`5 SUCCESS`); `cmd.exe /c "npm run build:ci"` OK con warnings de budget conocidos.
  - Proximos pasos: Revisar visualmente `Skills` con `EditMode Enabled`, validar V19 en CI/Testcontainers y confirmar que el frontend no se despliegue antes del backend con `/api/skills`.

- Fecha: 2026-05-15
  - Cambio: Se corrigio la resolucion local del storage documental para evitar que los documentos de `Formacion y certificaciones` parezcan no persistir despues de reiniciar. `DocumentStorageProperties` ahora resuelve paths relativos como `./runtime/documents` contra la raiz del workspace cuando el backend se levanta desde la raiz del repo o desde `backend/`, por lo que ambos modos apuntan a `runtime/documents`. Se agrego una spec corta, reporte de verificacion y revision adversarial para el bug.
  - Archivos: `backend/src/main/java/com/fernandogferreyra/portfolio/backend/config/DocumentStorageProperties.java`, `backend/src/test/java/com/fernandogferreyra/portfolio/backend/config/DocumentStoragePropertiesTest.java`, `docs/features/credential-document-local-persistence.md`, `docs/features/credential-document-local-persistence-verification.md`, `docs/features/credential-document-local-persistence-adversarial-review.md`, `DOCUMENTATION.md`, `docs/handoff-control-center.md`
  - Decision: Mantener filesystem local configurable y no cambiar contratos ni endpoints. Los paths absolutos configurados por `PORTFOLIO_DOCUMENT_STORAGE_PATH` se respetan; solo se estabiliza el caso relativo dentro del workspace.
  - Validacion: `cmd.exe /c "set JAVA_HOME=C:\Program Files\Java\jdk-17&& mvnw.cmd -Dtest=DocumentStoragePropertiesTest test"` OK (`3` tests); `cmd.exe /c "set JAVA_HOME=C:\Program Files\Java\jdk-17&& mvnw.cmd -DskipTests clean test-compile"` OK; `git diff --check` OK. Queda pendiente prueba manual end-to-end con subida, reinicio y preview.
  - Proximos pasos: Consolidar o re-subir documentos locales que hayan quedado en `backend/runtime/documents` si la DB local los referencia, y avanzar luego con el catalogo editable backend-first de `Skills`: `Nueva skill`, edicion completa, categorias dinamicas/eliminables y `Enfoque tecnico` derivado de categorias reales publicadas.

- Fecha: 2026-05-15
  - Cambio: Se corrigio la preview de documentacion en `Formacion y certificaciones` para que los PDFs no dependan del fallback posterior al error de imagen al refrescar con F5. Las credenciales con PDF ahora se renderizan directamente en iframe: los documentos verticales usan vista ajustada al ancho de pagina y los certificados horizontales usan vista ajustada a la altura para no quedar chicos centrados dentro del recuadro. Las imagenes mantienen el preview directo dentro del recuadro. Tambien se alineo la altura mobile del contenedor para que el preview no quede recortado por reglas heredadas.
  - Archivos: `frontend/src/app/components/credentials/credentials.component.ts`, `frontend/src/app/components/credentials/credentials.component.html`, `frontend/src/app/components/credentials/credentials.component.scss`, `frontend/src/app/components/credentials/credentials.component.spec.ts`, `DOCUMENTATION.md`
  - Decision: Mantener la solucion frontend-only y no introducir librerias de renderizado PDF. El backend sigue siendo source of truth de credenciales/documentos; la UI solo normaliza el encuadre visual del preview.
  - Validacion: `npx tsc -p tsconfig.app.json --noEmit` OK; `npx tsc -p tsconfig.spec.json --noEmit` OK; `cmd.exe /c "cd /d C:\Users\ferch\ProyectosIA\portfolio-profesional\frontend && npm test -- --watch=false --browsers=ChromeHeadless --include src/app/components/credentials/credentials.component.spec.ts"` OK (`3 SUCCESS`); `git diff --check` OK.
  - Proximos pasos: Revalidar visualmente `/credentials` en desktop despues de F5, especialmente las dos primeras credenciales verticales y las certificaciones horizontales de IA/Ciberseguridad.

- Fecha: 2026-05-15
  - Cambio: Se continuo `feature/edit-mode-skills-home` desde `develop` despues de mergear `#60`. `Inicio` y `Skills` ahora consumen bloques CMS propios con fallback local, y `EditMode` permite editar titulo/cuerpo/items desde las paginas publicas con editores contextuales cerca de cada bloque. `Skills` usa bloques `skill.<id>` publicados como fuente editable de copy/tags, permite agregar/quitar tags y ocultar una skill con `published=false` sin borrado definitivo. `Inicio` permite crear nuevas experiencias tecnicas como bloques `home.technical.*`, deriva `Stack y enfoque` desde skills publicadas y ya no duplica el resumen de credenciales porque esa informacion queda solo en `Formacion y certificaciones`. Se agrego la migracion `V18` para sembrar bloques `home.*`, `skills.hero` y todas las skills actuales en ES/EN, mas `POST /api/admin/content-blocks` para alta admin de bloques CMS. Para `Formacion y certificaciones`, los documentos de credenciales en borrador ahora tienen preview admin protegido por `/api/admin/credentials/{id}/document`, consumido como Blob autenticado desde Angular para evitar 401 por iframe sin interceptor; la preview intenta imagen primero y cae a iframe solo si falla, para no depender del `content-type` reportado por navegador/backend. `Proyectos` suma alta de proyecto como borrador y baja admin con confirmacion. Tambien se agrego el agente de proyecto `.opencode/agent/modo-edicion.md` para reutilizar esta logica en futuros sitios o paginas.
  - Archivos: `backend/src/main/resources/db/migration/V18__home_and_skills_content_blocks.sql`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/controller/admin/PublicContentBlockAdminController.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/controller/admin/CredentialAdminController.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/controller/admin/ProjectAdminController.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/dto/publiccontent/PublicContentBlockCreateRequest.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/mapper/publiccontent/PublicContentBlockMapper.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/repository/publiccontent/PublicContentBlockRepository.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/repository/projects/ProjectRepository.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/service/PublicContentBlockService.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/service/CredentialService.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/service/ProjectService.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/service/impl/PublicContentBlockServiceImpl.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/service/impl/CredentialServiceImpl.java`, `backend/src/main/java/com/fernandogferreyra/portfolio/backend/service/impl/ProjectServiceImpl.java`, `backend/src/test/java/com/fernandogferreyra/portfolio/backend/ApiIntegrationTest.java`, `frontend/src/app/services/public-content.service.ts`, `frontend/src/app/services/public-content-admin.service.ts`, `frontend/src/app/services/credential.service.ts`, `frontend/src/app/services/project-admin.service.ts`, `frontend/src/app/components/credentials/credentials.component.ts`, `frontend/src/app/components/credentials/credentials.component.html`, `frontend/src/app/components/credentials/credentials.component.scss`, `frontend/src/app/components/credentials/credentials.component.spec.ts`, `frontend/src/app/components/home/home.component.ts`, `frontend/src/app/components/home/home.component.html`, `frontend/src/app/components/home/home.component.scss`, `frontend/src/app/components/home/home.component.spec.ts`, `frontend/src/app/components/projects/projects.component.ts`, `frontend/src/app/components/projects/projects.component.html`, `frontend/src/app/components/projects/projects.component.scss`, `frontend/src/app/components/skills/skills.component.ts`, `frontend/src/app/components/skills/skills.component.html`, `frontend/src/app/components/skills/skills.component.scss`, `frontend/src/app/components/skills/skills.component.spec.ts`, `frontend/src/styles.scss`, `.opencode/agent/modo-edicion.md`, `docs/features/edit-mode-skills-home.md`, `docs/features/edit-mode-skills-home-verification.md`, `docs/features/edit-mode-skills-home-adversarial-review.md`, `DOCUMENTATION.md`, `docs/handoff-control-center.md`
  - Decision: Reutilizar `public_content_blocks` y sumar solo un alta admin generica para no abrir todavia una tabla dedicada de skills ni endpoints especificos por pagina. La metadata visual de skill (categoria, icono y nivel) queda en frontend por ahora; el copy, tags y publicacion quedan en CMS. El bloque de credenciales se retira de Home porque su source of truth y detalle ya viven en `Formacion y certificaciones`. La baja de proyectos elimina la entidad pero no los documentos asociados, para evitar borrar assets reutilizables por accidente. La preview admin de credenciales usa object URLs locales para conservar autenticacion sin exponer borradores por endpoints publicos.
  - Validacion: `npx tsc -p tsconfig.app.json --noEmit` OK; `npx tsc -p tsconfig.spec.json --noEmit` OK; `cmd.exe /c "cd /d C:\Users\ferch\ProyectosIA\portfolio-profesional\frontend && npm test -- --watch=false --browsers=ChromeHeadless && npm run build:ci"` OK (`45 SUCCESS`, build OK con warnings de budget conocidos); `cmd.exe /c "cd /d C:\Users\ferch\ProyectosIA\portfolio-profesional\backend && set JAVA_HOME=C:\Program Files\Java\jdk-17&& mvnw.cmd -DskipTests clean package"` OK; `cmd.exe /c "cd /d C:\Users\ferch\ProyectosIA\portfolio-profesional\backend && set JAVA_HOME=C:\Program Files\Java\jdk-17&& mvnw.cmd -DskipTests package"` OK tras el ajuste Blob; `git diff --check` OK. `mvnw.cmd test` completo queda para CI por dependencia de Docker/Testcontainers.
  - Proximos pasos: Revisar visualmente `Inicio`, `Skills`, `Formacion y certificaciones` y `Proyectos` con `EditMode Enabled`: editar hero/about/experiencia tecnica, crear una experiencia nueva, agregar/quitar tags en una skill, subir documento en una credencial borrador, crear/eliminar proyecto y confirmar que la vista publica refleja los cambios por idioma activo.
