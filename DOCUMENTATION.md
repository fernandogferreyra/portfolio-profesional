...
- Fecha: 2026-05-15
  ...
  - Archivos: `frontend/src/app/components/home/home.component.spec.ts`, `frontend/angular.json`, `DOCUMENTATION.md`, `docs/handoff-control-center.md`
  - Decision: Mantener los warnings de budget visibles, pero evitar que el umbral de error quede por debajo del tamano real ya aceptado por la UI actual. No se tocaron dependencias ni codigo funcional para resolver bloqueos locales de entorno.
  - Validacion: `cmd.exe /c "cd /d C:\Users\ferch\ProyectosIA\portfolio-profesional\frontend && npm test -- --watch=false --browsers=ChromeHeadless && npm run build:ci"` OK (`42 SUCCESS`; build OK con warnings de budget). `cmd.exe /c "cd /d C:\Users\ferch\ProyectosIA\portfolio-profesional\backend && set JAVA_HOME=C:\Program Files\Java\jdk-17&& mvnw.cmd -DskipTests package"` OK. `git diff --check` OK con warning CRLF/LF conocido en `frontend/angular.json`. `mvnw.cmd test` quedo bloqueado localmente por Docker/Testcontainers no disponible.
  - Proximos pasos: Usar el PR hacia `develop` como validacion final de CI backend completa, porque GitHub Actions corre en Ubuntu con Docker disponible para Testcontainers.
