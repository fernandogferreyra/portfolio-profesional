...
- Fix operativo local: el backend caia por `Flyway checksum mismatch` en `V16__projects_editable_detail_assets.sql` porque la migracion ya aplicada se habia modificado. Se restauro `V16` y el ajuste posterior pasa por `V17__projects_highlights_without_public_metrics.sql`. Con `mvnw.cmd spring-boot:run -Dspring-boot.run.profiles=dev`, Flyway valida 18 migraciones, aplica `V17` y Tomcat arranca en `8080`.
...
- Revisar visualmente la edicion extendida de `Proyectos`: editar metricas/secciones/puntos, subir multiples documentos y capturas/fotos, y confirmar que el detalle publico refleja todo antes de replicar el patron en `Skills` e `Inicio`.
- Antes de avanzar con `Skills` e `Inicio`, revisar visualmente que `Proyectos` no repita metricas ni stack en cards, que las secciones sigan visibles y editables por separado, que `Destacados` sea suficiente para resaltar, que el panel derecho muestre logo animado/nombre y no thumbnail de YouTube, que el carrusel solo aparezca con capturas reales, que el stack use iconos grandes consistentes y que la ruta activa del navbar tenga brillo neon sin perder legibilidad.
