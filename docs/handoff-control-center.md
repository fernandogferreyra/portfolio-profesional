...
- Implementar la siguiente etapa de `Skills` con catalogo backend-first: `Nueva skill`, `Editar`, nivel/icono/nombre/descripcion/categoria, categorias editables/eliminables con reasignacion a `Otras`, y `Enfoque tecnico` derivado de categorias publicadas reales incluyendo `Soft skills` cuando corresponda.
