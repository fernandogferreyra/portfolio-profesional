...
- En `feature/edit-mode-skills-catalog` se corrigio la causa local mas probable de documentos de `Formacion y certificaciones` que parecian no persistir tras reiniciar: el storage `./runtime/documents` ahora se resuelve contra la raiz del workspace si Spring arranca desde la raiz del repo o desde `backend/`, evitando que se alternen `runtime/documents` y `backend/runtime/documents`. Queda pendiente prueba manual end-to-end y consolidar/re-subir archivos locales que hayan quedado en el storage anterior.
- En la misma rama, `Skills` deja de usar el catalogo hardcodeado como fuente principal: V19 crea `skill_categories` y `skills` con seed ES/EN; backend expone `GET /api/skills`, `GET/POST/PATCH /api/admin/skills` y `POST/PATCH/DELETE /api/admin/skill-categories`. La UI de `Skills` en `EditMode` permite `Nueva skill`, `Editar`, nivel/icono/nombre/descripcion/categoria/tags/orden/publicacion, crear/editar/eliminar categorias y al eliminar una categoria el backend reasigna sus skills a `Otras`. `Enfoque tecnico` y `Home > Stack y enfoque` derivan de categorias/skills reales publicadas, incluyendo `Soft skills`.
- Ajuste UX posterior de `Skills`: `skills.hero` ya no se edita desde la pagina, `Enfoque tecnico` queda automatico, `EditMode` fuerza vista expandida para mostrar formularios reales, el carrusel animado no muestra controles de edicion, y `Nueva categoria` selecciona una categoria pendiente existente para evitar duplicados por clicks repetidos.
- Cierre de `Skills EditMode`: V20 agrega icono subido por documento, color principal editable por skill y baja admin de skills con accion visible junto a `Editar` y modal de confirmacion tematizado. Los iconos se suben con `purpose=skill-icon`, se sirven por `/api/skills/{id}/icon` y `DocumentService` desvincula skills si se borra un documento usado como icono.
- Pasada visual pre-deploy en la misma rama: navbar mas compacto, controles admin/theme/idioma/salida alineados, sin GitHub/LinkedIn en header; selector de themes compacto sin descripciones; `EX` negro/rojo puro, `CMD` blanco/negro, `WB` visible como `GREY`; nuevo theme `GRID` azul con rejilla; rejilla de fondo animada global y Contacto con tiles grandes de canales sobre grilla.
...
- En `fix/durable-cv-documents` se prepara el fix permanente para CV/documentos en deploy: `V21` agrega `document_contents` con binario durable en PostgreSQL, las nuevas subidas guardan copia DB ademas del filesystem, y las descargas publicas/admin caen a esa copia si el archivo fisico del container desaparece. Los documentos ya perdidos antes del fix no se pueden reconstruir; tras deployar esta rama hay que re-subir/asociar el CV una vez para crear la copia durable.
- En `fix/profile-photo-cv-replace` se agrega reemplazo real para documentos CMS sensibles: `contact.cv` y `site.profile-photo` borran el documento anterior cuando se asocia uno nuevo, `documentUrl` queda versionado con `v=<documentId>` y la descarga CMS usa `Cache-Control: no-store` para evitar que se abra el CV viejo por cache. Tambien se siembran bloques `site.profile-photo` ES/EN y Home `EditMode` permite actualizar la foto usada por Inicio y Header.
...
- Revisar visualmente `Skills` con `EditMode Enabled`: crear skill borrador, editar nivel/icono/nombre/descripcion/categoria/tags/orden, publicar, crear categoria, eliminar categoria y confirmar que las skills pasan a `Otras`.
- Si ya quedaron categorias duplicadas `Nueva categoria` por pruebas visuales previas, limpiarlas desde la vista expandida con `Eliminar categoria`; al borrarlas, las skills asociadas no se eliminan y pasan a `Otras`.
- Revalidar visualmente la subida de icono propio, selector de color, baja de skill con modal y limpieza de categorias duplicadas en `Skills` con `EditMode Enabled`.
- Validar V19/V20 en CI/Testcontainers antes de promocionar y no desplegar el frontend nuevo sin backend con `/api/skills` disponible.
- Para documentos locales ya subidos antes del fix de storage, si una credencial no encuentra el archivo fisico, consolidar manualmente `backend/runtime/documents` hacia `runtime/documents` o re-subir/asociar el documento; para sesiones futuras preferir `PORTFOLIO_DOCUMENT_STORAGE_PATH` absoluto.
